...
type ICVRepository interface {
	...
	GetApplicantInfo(ctx context.Context, applicantID int) (string, string, string, string, []domain.DbCV, []domain.DbExperience, []domain.DbEducationInstitution, error) //
...
func (repo *psqlCVRepository) GetApplicantInfo(ctx context.Context, applicantID int) (string, string, string, string, []domain.DbCV, []domain.DbExperience, []domain.DbEducationInstitution, error) {
	contextLogger := contextUtils.GetContextLogger(ctx)
	...
	err = repo.DB.QueryRow(`SELECT first_name, last_name, email, avatar_path FROM hnh_data.user_profile WHERE id = $1`, userID).Scan(&first_name, &last_name, &email, &path)
	if err != nil {
		return "", "", "", "", nil, nil, nil, err
	}
	...
	return first_name, last_name, email, pathToReturn, cvs, exp, edu, nil
}
