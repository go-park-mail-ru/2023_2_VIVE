...
CREATE TABLE hnh_data.education_institution (
    ...
    major_field TEXT NOT NULL
        CONSTRAINT major_field_is_not_empty CHECK (length(major_field) > 0),
...
ALTER TABLE cv
    ...
    ADD COLUMN middle_name TEXT 
        CONSTRAINT middle_name_is_not_empty CHECK (length(middle_name) > 0),
    ADD COLUMN gender TEXT NOT NULL 
        CONSTRAINT gender_is_not_empty CHECK (length(gender) > 0),
